...
// PipeClamp-1-20260115A.scad

// First version: 2026-01-15(Thu) 12:48 JST / 2026-01-15(Thu) 03:48 UTC
// Prev update: 2026-01-15(Thu) 21:41 JST / 2026-01-15(Thu) 12:41 UTC
// Last update: 2026-01-16(Fri) 05:16 JST / 2026-01-15(Thu) 20:16 UTC
...
tdepth = 3.0;                       // 文字を掘る深さ（mm）
text   = "PipeClamp-1-20260115A3";

hole_pos = [
  [ -30.0, 0,   0.0, 4.5 ],  [ -30.0, 0,  12.5, 4.5 ],  [ -30.0, 0, -12.5, 4.5 ],
  [  30.0, 0,   0.0, 4.5 ],  [  30.0, 0,  12.5, 4.5 ],  [  30.0, 0, -12.5, 4.5 ]
];
...
module main() {
    difference() {
        union() {
            ...
            // 半分の円柱の頂点側の「板」になる側の立方体
            translate([0, od/2, 0]) cube([bw, t2, len], center=true);
        }
        ...
        // テキストを掘る
        translate([0, od/2+t2/2-tdepth, -7.0])  // 「板」の表面から tdepth だけ深い場所に移動
          rotate([90, 0, 180]) // XZ平面に刻印するため、通常とは異なり板の中から板の表面に向かって裏返しに文字を掘る
